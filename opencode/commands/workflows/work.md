...
Use **todowrite** to create a todo entry for every story in prd.json:
...
**Track dependencies in prd.json directly** - the depends_on field shows which stories must complete first.
...
while (executable stories remain):
  ...
  2. UPDATE prd.json + Task system:
     ...
     - Update todo status to "in_progress" using todowrite
  ...
  4. LOAD relevant skills:
     ...
     - Use the **skill** tool to load each skill for implementation guidance
  ...
  9. UPDATE prd.json + Task system (MANDATORY — ALWAYS runs after commit):
     ...
     - Update todo status to "completed" using todowrite
...
3. Update todo description to note the blocker using todowrite
...
Use the **skill** tool to load these skills before implementation:
```
Use the **skill** tool to load: frontend-design, vercel-react-best-practices
```
