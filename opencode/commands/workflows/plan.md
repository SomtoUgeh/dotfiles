...
Use the **question** tool to ask which brainstorm to use, or whether to proceed without one.
...
- Use the **task** tool with subagent_type "explore" to run repo-research-analyst
...
- Use the **task** tool with subagent_type "explore" to run best-practices-researcher
- Use the **task** tool with subagent_type "explore" to run framework-docs-researcher
...
- [ ] If no URL/ticket, use the **question** tool: "Is there a Jira ticket for this work? (e.g., HAW-1234, or 'none')"
...
- `FMFX-1456-fix-checkout-race-condition`
...
- Use the **task** tool with subagent_type "explore" to run spec-flow-analyzer
...
1. Use the **skill** tool to load `breadboarding` as methodology reference
...
After writing spec.md and prd.json, use the **question** tool:
