"""Local inert-content scanner; scan_repo.sh selects the isolated runtime."""
import json
...
module_path = Path(__file__).with_name("worm_guard_patterns.py")
...
GROUPS = (
    ...
    ("binary-indicator", "Indicator bytes in binary content"),
    ("generic-artifact", "Possible helper filenames or ignore rules"),
    ("obfuscation-seed", "Numeric obfuscator seeds"),
    ("host", "Workstation process and startup review"),
    ("signing", "Commit signature headers absent"),
)
...
class Scanner:
    def __init__(self, root, scanner_path, include_generated=False, details=False, workstation=False, check_signing=False):
        self.root = root
        self.scanner_path = scanner_path
        self.include_generated = include_generated
        self.details = details
        self.workstation = workstation
        self.check_signing = check_signing
        self.signature_headers_checked = 0
...
def scan_file(scanner, path):
    ...
    try:
        if any(path.resolve(strict=True).samefile(trusted) for trusted in (scanner.scanner_path, Path(__file__), module_path)):
    ...
    if path.name == ".env":
        scanner.advisory(path, None, ".env file present; verify ignore rules and whether it was committed; presence alone does not establish exposure")
    ...
    if text is TEXT_DECODE_ERROR:
        ...
        patterns.scan_binary(scanner, path, data)
...
def scan_git_objects(scanner, view, gitdir, hash_bytes, names, signing_commits):
        ...
        with git_command(view, ["cat-file", "--batch"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=errors) as process:
            ...
            try:
                for oid, kind, size in objects:
                    ...
                    if kind == b"tree":
                    ...
                    elif kind == b"commit" and oid in signing_commits:
                        scanner.signature_headers_checked += 1
                        headers = data.split(b"\n\n", 1)[0].splitlines()
                        if not any(line.startswith((b"gpgsig ", b"gpgsig-sha256 ")) for line in headers):
                            scanner.finding("signing", location, None, "commit has no embedded signature header; this does not establish tampering")
                    elif kind == b"blob":
                        scanner.git_blobs += 1
                        ...
                        for name in sorted(names.get(oid.decode("ascii"), {"unreferenced-blob"})):
                            ...
                            if text is not None:
                                ...
                                patterns.scan_binary(scanner, display, data)
                else:
                    completed = True
            except (OSError, ValueError):
    ...
    with tempfile.TemporaryDirectory(prefix="worm-guard-git-") as temporary:
        ...
        if log is not None:
            for raw_line in log.splitlines():
                ...
                if scanner.check_signing and len(signing_commits) < 20:
                    signing_commits.add(commit)
        ...
        scan_git_objects(scanner, view, common, 32 if object_format == "sha256" else 20, names, signing_commits)
    return paths
...
def print_summary(scanner, campaign, review):
    """Explain the verdict and next steps before showing technical evidence."""
    total = campaign + review
    if scanner.error_count:
        result = "INCOMPLETE - some requested checks could not finish (exit 2)."
    elif total:
        result = "REVIEW NEEDED - scan finished; findings need a human check (exit 1)."
    else:
        result = "NO KNOWN INDICATORS - scan finished within the stated scope (exit 0)."
    print("\nResult: " + result)
    print("Target: " + json.dumps(str(scanner.root), ensure_ascii=True))
    print("Campaign matches: " + str(campaign) + "; Review signals: " + str(review)
          + "; Inspection errors: " + str(scanner.error_count))
    if campaign:
        print(str(campaign) + " matches contain known worm-related markers. This does not mean "
              + str(campaign) + " infected files: one file can match several checks.")
        print("Scanner rules, test samples and documentation can contain these same markers; inspect the content before deciding.")
    if review:
        print(str(review) + " other signals need context; ordinary code, formatting and installed tools can trigger them. They do not establish infection.")
    print("\nWhat to do next:")
    if scanner.error_count:
        print("  Resolve the inspection errors listed below, then rerun. Findings already reported still need review.")
    if scanner.finding_counts["oauth-token"]:
        print("  Check the reported token's revocation status. Revoke it if still active; deleting a file does not revoke a token.")
    if campaign:
        print("  Review campaign match locations first. Do not execute suspicious files or delete files just because they matched.")
    if review:
        print("  Review the remaining signal groups and record which are expected or need investigation.")
    if not total and not scanner.error_count:
        print("  No follow-up is indicated by these checks. Excluded content and other malware are not covered.")
    if total:
        print("  Repeating the scan will not clear findings; the scanner does not remember earlier human reviews.")
    if scanner.workstation and os.geteuid() == 0:
        print("  Workstation checks are running as root. Rerun without sudo to check your own startup files and crontab.")
    if not scanner.details:
        print("  Use --details for every matching location, error and context advisory.")


def print_coverage(scanner, history_checked):
    """Keep scope and omissions explicit regardless of the finding count."""
    print("\nScan coverage:")
    print(
    ...
    print("Workstation checks: " + ("requested; processes, current-user startup files/cron and macOS launch entries (see errors for omissions)" if scanner.workstation else "not requested; use --workstation"))
    if scanner.check_signing:
        print("Signature headers checked: " + str(scanner.signature_headers_checked)
              + "; up to 20 recent commits across local refs per repository; cryptographic validity is not verified.")
...
def print_findings(scanner):
    """Limit only the default display; detailed output retains all evidence."""
    for is_campaign, heading in ((True, "Campaign match locations"), (False, "Review signals (not proof of infection)")):
        ...
        for key, title in groups:
            ...
            items = scanner.findings[key] if scanner.details else scanner.findings[key][:3]
            for item in items:
                print_item(scanner, item)
            if count > len(items):
                print("    " + str(count - len(items)) + " more location(s); "
                      + "use --details to show all")
    if scanner.advisories:
        print("\nContext advisories (not findings): " + str(len(scanner.advisories)))
        if scanner.details:
            for item in scanner.advisories:
                print_item(scanner, item)
    if scanner.errors:
        print("\nInspection incomplete - errors to resolve:")
        errors = scanner.errors if scanner.details else scanner.errors[:3]
        for path, reason in errors:
            print("    path=" + display_path(scanner.root, path) + " reason=" + reason)
        if scanner.error_count > len(errors):
            print("    " + str(scanner.error_count - len(errors)) + " more error(s); use --details to show all")
...
def render(scanner, history_checked):
    ...
    print_summary(scanner, campaign, total - campaign)
    print_coverage(scanner, history_checked)
...
def main():
    ...
    if not root.is_dir() or not scanner_path.is_file():
        ...
        return 20
    scanner = Scanner(root, scanner_path, sys.argv[3] == "1", sys.argv[4] == "1", sys.argv[5] == "1", sys.argv[6] == "1")
    try:
        ...
        if scanner.workstation:
            scanner.progress("checking workstation startup and processes", Path.home())
            host_spec = importlib.util.spec_from_file_location("worm_guard_host", module_path.with_name("worm_guard_host.py"))
            host = importlib.util.module_from_spec(host_spec)
            host_spec.loader.exec_module(host)
            host.scan_host(scanner, Path.home())
