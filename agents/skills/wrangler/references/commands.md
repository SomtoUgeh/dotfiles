# Wrangler command examples

Read the entrypoint first. Verify examples against the installed version and current official docs; execute mutations only within user authorization.
...
```jsonc
{
  ...
  // Lifecycle for Durable Objects exported by this Worker
  "exports": {
...
Use declarative `exports` for new Durable Object deployments. Existing migration-based deployments remain supported; continue their migration tags because Wrangler rejects configurations that combine Durable Object `exports` and `migrations`.
...
# Supply the full vector matching the index dimensions.
```
...
The documented Hyperdrive create/update CLI accepts passwords and connection
strings as arguments; expanding an environment variable still exposes its value
in argv. Do not invent a password-stdin flag. For secret-bearing creation or
rotation, use the authenticated Cloudflare dashboard, or an existing approved
API integration that supplies secrets in the request body without logging them.

See [Hyperdrive commands](https://developers.cloudflare.com/hyperdrive/reference/wrangler-commands/)
and [credential rotation](https://developers.cloudflare.com/hyperdrive/configuration/rotate-credentials/).

```bash
wrangler hyperdrive list
wrangler hyperdrive get <HYPERDRIVE_ID>
# Only when deletion is authorized:
...
wrangler workflows trigger my-workflow '{"key": "value"}'
...
```bash
# Create stream/sink resources and a connecting pipeline interactively
...
# Update a legacy pipeline only (new pipelines use streams/sinks/SQL)
...
```bash
# Account store operations require --remote
...
```bash
# Create a remote secret; Wrangler securely prompts for its value
...
# Update value or scopes (secure prompt for value when --value is omitted)
wrangler secrets-store secret update <STORE_ID> --secret-id <SECRET_ID> --scopes workers --remote

# Delete secret from store
wrangler secrets-store secret delete <STORE_ID> --secret-id <SECRET_ID> --remote
```

Remote list output provides the store and secret IDs. Without `--remote`, secret commands operate on the local Secrets Store used by `wrangler dev`; local stores are created automatically on first use.
...
The tested plugin 1.1.4 requires Vitest `^4.1.0`. Recheck peer dependencies on upgrades. Use `"type": "module"` in package.json or `vitest.config.mts` for this ESM-only plugin.
...
| Issue | Solution |
|-------|----------|
| `command not found: wrangler` | Resolve the project executable first; install only for authorized implementation |
...
2. **Check provisioning support per binding**: Only omit IDs where installed Wrangler supports automatic provisioning; review proposed resource creation before deploy.
...
5. **Set `compatibility_date`**: Update deliberately with tests for compatibility changes.
