...
Read this reference only when an existing widget needs secret retrieval or a secret-store write. The sitekey may come from the user or verified local discovery. Reuse a working secret binding without retrieving its value; frontend-only repairs do not need this flow. Follow the parent skill’s authorization and integration contracts.
