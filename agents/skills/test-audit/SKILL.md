...
Do not edit source or tests while a test run is in progress in the checkout.
Resolve commands from that repository: root and scoped `AGENTS.md`, CI config,
and package scripts.

1. Run the smallest owner and sibling tests with the repository's own test
   command, filtered to those paths. Use a documented heavy or remote runner
   only when that command cannot prove the contract or repository policy
   requires it.
2. For removed source greps or plan assertions, run the executable script or
   dry-run that owns the real contract.
3. Run the repository's formatter on the touched files, then `git diff --check`.
4. If the repository documents a changed-path gate, run it for the changed
   paths. Otherwise the focused tests, formatter, and `git diff --check` are
   the gate. Report any required check the repository does not provide.
5. Inspect `git diff --numstat`; report production/tooling separately from
   tests and test support.
6. After final audit edits, load `code-review` and run closeout on the diff.

## Landing and continuation

Commit, push, open a PR, or land only when authorized. Follow the repository's
documented PR flow when it has one; otherwise follow the active harness's git
workflow. Land one coherent PR at a time; after landing, refresh from current
