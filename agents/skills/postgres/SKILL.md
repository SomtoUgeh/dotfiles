...
Start with the provider, deployment model, PostgreSQL version, workload, and constraints already present in the repository or supplied by the user. Keep provider-specific advice within that provider's section. When the user asks for a new provider recommendation, compare current operational needs, regions, extensions, compliance, migration cost, pricing, and existing team tooling before recommending one.
...
Use these references only when the project already uses PlanetScale or the user is evaluating it.
