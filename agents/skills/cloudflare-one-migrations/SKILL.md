...
- ZPA bypass means split-tunnel bypass in Cloudflare, not an Access `bypass` decision. Bypass rules map to WARP [Split Tunnel](https://developers.cloudflare.com/cloudflare-one/team-and-resources/devices/cloudflare-one-client/configure/route-traffic/split-tunnels/) exclude entries. Configure the appropriate profile through the dashboard or the documented [device profile excludes API](https://developers.cloudflare.com/api/resources/zero_trust/subresources/devices/subresources/policies/subresources/custom/). Read and preserve existing entries before an update; reconcile Include mode separately instead of assuming every profile uses Exclude mode.
...
- Retrieve the current [account limits](https://developers.cloudflare.com/cloudflare-one/account-limits/) and verify the target application type and account entitlement before deciding how many destinations fit in an app. Do not assume a historical default or a guaranteed limit increase. If the active limit requires splitting an app segment, preserve identical policies and include the resulting object count in reconciliation.
