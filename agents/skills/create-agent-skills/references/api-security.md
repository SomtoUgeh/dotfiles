...
<the_solution>
Use `~/.agents/scripts/secure-api.sh` - a wrapper that loads credentials internally.

<for_supported_services>
```bash
# ✅ GOOD - No credentials visible
~/.agents/scripts/secure-api.sh <service> <operation> [args]

# Examples:
~/.agents/scripts/secure-api.sh facebook list-campaigns
~/.agents/scripts/secure-api.sh ghl search-contact "email@example.com"
```
</for_supported_services>
...
1. **Add operations to the wrapper** (`~/.agents/scripts/secure-api.sh`):
...
3. **Add credential placeholders to `~/.agents/.env`** using profile naming:

```bash
# Check if entries already exist
grep -q "YOURSERVICE_MAIN_API_KEY=" ~/.agents/.env 2>/dev/null || \
  echo -e "\n# Your Service - Main profile\nYOURSERVICE_MAIN_API_KEY=\nYOURSERVICE_MAIN_ACCOUNT_ID=" >> ~/.agents/.env

echo "Added credential placeholders to ~/.agents/.env - user needs to fill them in"
```
...
1. **Check for saved profile:**
   ```bash
   ~/.agents/scripts/profile-state get yourservice
   ```

2. **If no profile saved, discover available profiles:**
   ```bash
   ~/.agents/scripts/list-profiles yourservice
   ```
...
5. **Save user's selection:**
   ```bash
   ~/.agents/scripts/profile-state set yourservice <selected_profile>
   ```
...
7. **Make API call with profile:**
   ```bash
   ~/.agents/scripts/secure-api.sh yourservice:<profile> list-items
   ```
...
```bash
~/.agents/scripts/secure-api.sh yourservice:<profile> <operation> [args]

# Examples:
~/.agents/scripts/secure-api.sh yourservice:main list-items
~/.agents/scripts/secure-api.sh yourservice:main get-item <ITEM_ID>
```
...
Usage:
```bash
echo '{"name":"value"}' | ~/.agents/scripts/secure-api.sh service create-item
```
</post_with_json_body>
...
<credential_storage>
**Location:** `~/.agents/.env` (global for all shared skills, accessible from any directory)
...
```bash
set -a
source ~/.agents/.env 2>/dev/null || { echo "Error: ~/.agents/.env not found" >&2; exit 1; }
set +a
```
</credential_storage>
...
3. **Auto-create credential placeholders** - add empty fields to `~/.agents/.env` immediately when creating the skill
4. **Keep credentials in `~/.agents/.env`** - one central location, works everywhere
...
grep -q "YOUR_API_KEY=" ~/.agents/.env && echo "API key configured" || echo "API key missing"
```
</testing>
