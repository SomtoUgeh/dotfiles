...
### Using structured user-question tool
...
**When external service detected**, ask using structured user-question tool:
...
Prefer the bundled initializer when creating a normal scaffold:

```bash
uv run python ~/.agents/skills/create-agent-skills/scripts/init_skill.py {skill-name} --path ~/.agents/skills
```

Then customize the generated files and delete unused example resources.
...
# If needed:
mkdir -p ~/.agents/skills/{skill-name}/templates  # for output structures
mkdir -p ~/.agents/skills/{skill-name}/scripts    # for reusable code
```
...
```bash
uv run python ~/.agents/skills/create-agent-skills/scripts/quick_validate.py ~/.agents/skills/{skill-name}
```
...
## Step 9: Create Runtime Entry Points

Create runtime-specific wrappers only when needed:

- Shared skill source: `~/.agents/skills/{skill-name}/SKILL.md`
- Claude command wrappers: keep in `~/.claude/commands/` or the repository's Claude command folder
- OpenCode commands: keep in `~/.config/opencode/commands/`
- Codex: use skills directly; do not invent Claude-style slash-command metadata
...
If the skill needs to be shared as an archive:

```bash
uv run python ~/.agents/skills/create-agent-skills/scripts/package_skill.py ~/.agents/skills/{skill-name} ./dist
```
</process>
...
- [ ] Runtime entry point exists if this skill needs one
