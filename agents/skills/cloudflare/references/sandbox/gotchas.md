...
A Sandbox session is a shell context within a shared container, not a security boundary between tenants. Container files, including /workspace, are ephemeral unless backed up or mounted to durable storage. Preview URLs are credentials, not proof of user authorization. Do not blindly retry a command after an ambiguous timeout.
