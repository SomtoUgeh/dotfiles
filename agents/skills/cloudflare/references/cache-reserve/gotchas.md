...
**Solution:** Ensure minimum TTL of 10+ hours (`Cache-Control: public, max-age=36000`), add Content-Length header, keep personalized responses ineligible; configure a separate public asset route, and set `Vary: Accept-Encoding` (not *)
...
**Solution:** Disable Cache Reserve first, confirm the disabled setting by reading it back, then clear data (can take up to 24 hours)
...
| Maximum file size | Same as R2 limits | Check current Cache Reserve/CDN asset limits |
...
5. Is Set-Cookie header present?
   → Yes: Keep the response private; separate truly public assets
