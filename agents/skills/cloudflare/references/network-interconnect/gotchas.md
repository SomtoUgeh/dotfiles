...
**Solution:** Use the slots API and [current setup documentation](https://developers.cloudflare.com/network-interconnect/get-started/) for site codes
...
Validate fields against the current SDK/API schema. No documented `validate_only` creation option is exposed by SDK 7.1.0; do not use a creation call as a dry run.

### Rate Limiting

**Limit:** 1200 requests/5min per user/account API quota; consult response headers
...
Current interconnect APIs support GCP pairing-key creation and connection status. Inspect the installed SDK for supported fields; GCP-side VLAN attachment state remains a separate provider operation.
