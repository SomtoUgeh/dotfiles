...
Use the generated runtime types and project-local Wrangler. A send_email binding is for outbound sends; forwarding inbound email does not require one. Enable domain routing or change DNS only when authorized for the task. Destination verification is an account-scoped API, while routing rules are zone-scoped.
