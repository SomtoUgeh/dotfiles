...
|----------|-------------|----------|
| API Metering | requests, bytes, compute_units | customer_id |
...
// SELECT index1 AS customer_id, SUM(double2 * _sample_interval) AS compute_units
...
// SELECT blob1, quantileExactWeighted(0.95)(double1, _sample_interval) AS p95_ms FROM perf GROUP BY blob1
```
...
| Assume isolate memory is a durable aggregate | Write points directly or aggregate in durable storage |
| Expose a SQL API token to the browser | Query from a trusted Worker/backend using a secret |
...
4. **Handle sampling** - Use ratios (avg_latency = SUM(latency * _sample_interval)/SUM(count * _sample_interval))
