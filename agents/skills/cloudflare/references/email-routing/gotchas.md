...
The SMTP envelope sender is not authenticated identity. A sender allowlist alone does not prevent spoofing; rely on validated provider authentication results and appropriate application authorization. Do not trust arbitrary Authentication-Results substring matches. A raw MIME stream is single-use and has no arrayBuffer() method: use new Response(message.raw).arrayBuffer().
