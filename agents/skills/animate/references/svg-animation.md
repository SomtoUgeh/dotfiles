...
- **Path commands:** `M` move (no draw), `L` line, `Z` close. Uppercase = absolute, lowercase = relative. Use `Z` for intentionally closed contours; leave open strokes and line drawings open. Closing an open path adds an unwanted return segment.
- Zero-area rectangles/circles have no painted area. A zero-length path can still paint a dot with round or square linecaps; do not rely on coincident endpoints to hide a stroke.
...
```css
/* Apply class="draw-path" and pathLength="100" to each target path. */
.draw-path { stroke-dasharray: 100 110; stroke-dashoffset: 100; animation: draw 0.6s ease forwards; }
@keyframes draw { to { stroke-dashoffset: 0; } }
```

- **`pathLength="100"`** on the path normalizes its length so you work in normalized numeric units (percentages resolve against the viewport, not pathLength) and can share values across paths of different real lengths.
...
Motion gives SVG elements element-centered transforms by default. For a viewBox-relative pivot, set both `transformBox` and `transformOrigin` explicitly, then inspect the computed pivot in the installed version:

```jsx
<motion.g style={{ transformOrigin: "76.3px 69.5px", transformBox: "view-box" }}
...
Containment can limit how far rendering work propagates, and a 3D transform may
influence layer promotion, but both have browser-dependent costs and behavior.
Use them only after profiling identifies the affected elements; retained layers
consume memory.
